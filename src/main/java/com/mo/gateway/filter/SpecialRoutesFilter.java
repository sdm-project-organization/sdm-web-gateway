...
import com.mo.gateway.model.AbTestingRoute;
...
public class SpecialRoutesFilter extends ZuulFilter {
    ...
    public String filterType() {
        return filterUtils.ROUTE_FILTER_TYPE; // 경로필터
    }
    ...
    public Object run() {
        ...
        logger.debug("route filter header check : " + ctx.getRequest().getHeader(FilterUtils.CORRELATION_ID));
        logger.debug("route filter header check : " + ctx.getRequest().getHeader(FilterUtils.AUTHORIZATION));

        /*
        // SpecialRoutes 서비스호출을 실행하여 이 조직에 대한 라우팅
        AbTestingRoute abTestRoute = getAbRoutingInfo(filterUtils.getServiceId());

        // 경로 가중치를 가져와 난수를 생성하고 요청을 대체 서비스로 전달할지 경정
        if (abTestRoute!=null && useSpecialRoute(abTestRoute)) {

            // 라우팅 레코드가 있다면 SpecialRoutes 서비스가 지정한 서비스 위치의 완전한 URL을 만듬
